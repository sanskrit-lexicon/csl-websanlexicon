...
/* webtc/dal_sqlite.php  
  ...
  dal_krm1(key) from 'krm' return array of records (key1,lnum1,data1)
            where key1 = key
  dal_krm2(L1,L2) from 'krm' return array of records (key1,lnum1,data1)
            where L1 <= lnum <= L2
*/
...
function dal_krm_sql($sql) {
// General query on 'krm' database. Table assumed in $sql
$db = "../sqlite/krm.sqlite";
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_krm1($key) {
...
$db = "sqlite:../sqlite/krm.sqlite";
// Create (connect to) SQLite database in file
$file_db = new PDO($db);
// Set errormode to exceptions
$file_db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
$sql = "select * from krm where key='$key' order by lnum";
$result = $file_db->query($sql);
//echo "<p>Using PDO...</p>";
$ansarr=array();
foreach($result as $m) {
...
function dal_krm2($L1,$L2) {
// returns an array of records, one for each L-value in the range
// $L1 <= $L <= $L2
// each record is an array with three elements: key,lnum,data
$db = "../sqlite/krm.sqlite";
$sql="select * from krm where  $L1 <= lnum and lnum <= $L2  order by lnum"; 
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_krm3($key) {
// returns an array of records, which start like $key
// each record is an array with three elements: key,lnum,data
$db = "../sqlite/krm.sqlite";
$sql = "select * from krm where key LIKE '$key%' order by lnum";
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_krm4a($lnum0,$max) {
$sql = "select * from krm where (lnum < '$lnum0') order by lnum DESC LIMIT $max";
$recarr = dal_krm_sql($sql);
return $recarr;
}
function dal_krm4b($lnum0,$max) {
$sql = "select * from krm where ('$lnum0' < lnum) order by lnum LIMIT $max";
//$sql = "select * from `krm` where \"$lnum0\" < `lnum`  order by `lnum`  LIMIT $max";
$recarr = dal_krm_sql($sql);
return $recarr;
}
