readme.txt for krmweb download.  
May 28, 2014
Jim Funderburk
...
a user to install on a local computer a web-browser display of krm data.

unzip the krmweb.zip archive. This creates a folder called 'web'.
Rename 'web' to something such as 'krm'.
...
Move the 'krm' folder to the 'htdocs' subfolder of your Apache installation.
...
Then, use the url <home>/krm/ to get the home page of the krm displays;
for instance, http://127.0.0.1:4001/krm/.

For server2go, you may need to slightly adjust the pms_config.ini file to
the one contained in the server2go subdirectory of this krmweb download.


For Mac installation, move the krm folder to the Sites folder, and make 
sure web sharing is turned on.
You may need to changed the permissions on the krm 
folder and its subfolders and directories; e.g., in a terminal, change to
the Sites folder and 'chmod -RE 0777 krm'. (you may need to use sudo,
e.g., 'sudo chmod -RE 0777 krm').
