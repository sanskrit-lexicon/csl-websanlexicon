readme.txt for skdweb download.  
October 4, 2013. Jim Funderburk
...
a user to install on a local computer a web-browser display of skd data.

unzip the skdweb.zip archive. This creates a folder called 'web'.
Rename 'web' to something such as 'skd'.
...
Move the 'skd' folder to the 'htdocs' subfolder of your Apache installation.
...
Then, use the url <home>/skd/ to get the home page of the skd displays;
for instance, http://127.0.0.1:4001/skd/.

For server2go, you may need to slightly adjust the pms_config.ini file to
the one contained in the server2go subdirectory of this skdweb download.


For Mac installation, move the skd folder to the Sites folder, and make 
sure web sharing is turned on.
You may need to changed the permissions on the skd 
folder and its subfolders and directories; e.g., in a terminal, change to
the Sites folder and 'chmod -RE 0777 skd'. (you may need to use sudo,
e.g., 'sudo chmod -RE 0777 skd').
