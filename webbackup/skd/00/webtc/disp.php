...
// Aug 23, 2017 skd specific code (<mark>
*/
...
function basicDisplay($key,$matches,$filterin) {
 global $row,$row1,$pagecol,$inSanskrit,$dbg,$elattribs;
 global $inkey2;
 global $parentEl;
...
function line_adjust($line) {
 ...
 /* May 22, 2017. Handling '<br/>'
    Examples: akzapAda  (for - <br/>)
    akzamAlikopanizad  for replace <br/> with space.
 */ // removed for vcp
 //$line = preg_replace('|- <br/>|','',$line);
 //$line = preg_replace('|<br/>|',' ',$line);
 /* May 22, 2017. ^x -> <sup>x</sup> 
   The values of x are: a,b,c,d,0,2,th
   Example agastyasaMhitA.
 */ // removed for vcp
...
function sthndl($xp,$el,$attribs) {
 ...
 global $parentEl;
  if (preg_match('/^H.+$/',$el)) {
   ...
   // for vcp
   if ($n == 'Picture') {
    $row .= "&nbsp;<span style='font-size:smaller;'>(Picture)</span>";
   }
   if (($n == '2')) {
...
// echo "endhndl, $el, $inSanskrit\n";
 global $row,$row1,$pagecol,$inSanskrit,$elattribs;
 global $inkey2;
 global $parentEl;
  $parentEl = "";
  if ($el == "s") {
