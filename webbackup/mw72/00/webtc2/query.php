...
function smatchkey($fp,$lastLnum,$regexp,$max,$transLit) {
 $dbg = False;
 if ($dbg) {
  $fplog = fopen('query_log.txt','w');
  fwrite($fplog,"smatchkey regexp = {{$regexp}}\n");
 }

 $ntot = 0;
 fseek($fp,$lastLnum,0); // reposition
 //$line=fgets($fp); // Jan 10, 2016
 $ans=array();
 if (!feof($fp)) {
  $line=fgets($fp);
 }else {
 ...
 if ($dbg) {
  $ntry=0;
  fwrite($fplog,"Initial line = $line\n");
 }
 ...
 while ($line) {
  ...
  if ($dbg) {
   if($ntry < 10) {fwrite($fplog,"smatchkey: liney=$liney\n");}
   $ntry++;
  }
  if (preg_match("/$regexp/",$liney)) {   
   $linex=$line;
