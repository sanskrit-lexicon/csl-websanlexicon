...
// modified Oct 21, 2013 for bor.  
// each line of query_dump.txt is of form
// key<TAB>sanskrit_text<TAB>other_text
$dbg = FALSE;
...
if ($dbg) {
 dbgprint($dbg,"filter = $filter\n");
 dbgprint($dbg,"filename = $filename\n");
 dbgprint($dbg,"lastLnum = $lastLnum\n");
 dbgprint($dbg,"max = $max\n");
 dbgprint($dbg,"outopt = $outopt\n");
 dbgprint($dbg,"opt_sregexp = $opt_sregexp\n");
 dbgprint($dbg,"opt_sword = $opt_sword\n");
 dbgprint($dbg,"opt_stransLit = $opt_stransLit\n");
 dbgprint($dbg,"opt_regexp = $opt_regexp\n");
 dbgprint($dbg,"opt_word = $opt_word\n");
 dbgprint($dbg,"sopt_case = $sopt_case\n");
}
...
$dbg=FALSE;
if ($dbg) {
 echo "<p>$dbg. opt_sregexp=$opt_sregexp<br>\n";
 echo "opt_sword = $opt_sword<br>\n";
 echo "word = $word<br>\n";
 echo "opt_regexp = $opt_regexp<br>\n";
 echo "sopt_case = $sopt_case<br>\n";
 echo "</p>\n";
}
...
if ($word !="") {
 ...
 if ($dbg) {
  echo "<p>nmatches = " . count($matches) . " </p>\n";
  echo "dbg: stopping\n";
  exit(0);
 }
...
//    display_outopt3();
  ...
  $disp = display_outopt4($matches,$word,$slpword,$search_regexp);
}else {
...
function matchkey($fp,$lastLnum,$regexp,$max,$opt,$word) {
 $dbg=FALSE;
 if ($dbg) {
  echo "<p>matchkey: $lastLnum,$regexp,$max,$opt,$word</p>\n";
 }
 $ntot=0;
 if (!($word)) {
 ...
 while ($line) {
  $nline++;
  list($a,$b,$c) = preg_split("|\t|",$line);
  $linex="";
  //$liney=$line;
  $liney = "\t $a $c "; // the English parts. \t expected
  if ($dbg and ($nline < 5)) {
   echo "<p>liney = $liney</p>\n";
  }
...
function smatchkey($fp,$lastLnum,$regexp,$max,$transLit) {
 ...
 //dbg: fclose($fplog);
 $dbg=FALSE;
 dbgprint($dbg,"smatchkey: $lastLnum,$regexp,$max,$transLit\n");
 $ntot = 0;
 //fseek($fp,$lastLnum,0); // reposition
 fseek($fp,$lastLnum); // reposition
 $ans=array();
 if (!feof($fp)) {
  $line=fgets($fp);
 }else {
 ...
 while ($line) {
  ...
  //$liney=" " . $a . " \t"; // 
  $liney = $b . " \t";
  if($ntry < 10) {dbgprint($dbg,"smatchkey: line=$line\n");}
  $ntry++;
  if (preg_match("/$regexp/",$liney)) {   
   $linex=$line;
  }else {
   if($ntry <= 10) {dbgprint($dbg,"prev line does not match\n");}
  }
  if ($linex !="") {
...
function translate_string2SLP($transLit,$keyin) {
 ...
 //echo "translate_string2SLP: $transLit,$keyin => $key<br/>";
 return $key;
}
...
function display_outopt4 ($lines,$word,$sword,$search_regexp) {
 $nx=0;
