readme.txt for schweb download.  
March 20, 2014. Jim Funderburk
...
a user to install on a local computer a web-browser display of sch data.

unzip the schweb.zip archive. This creates a folder called 'web'.
Rename 'web' to something such as 'sch'.
...
Move the 'sch' folder to the 'htdocs' subfolder of your Apache installation.
...
Then, use the url <home>/sch/ to get the home page of the sch displays;
for instance, http://127.0.0.1:4001/sch/.

For server2go, you may need to slightly adjust the pms_config.ini file to
the one contained in the server2go subdirectory of this schweb download.


For Mac installation, move the sch folder to the Sites folder, and make 
sure web sharing is turned on.
You may need to changed the permissions on the sch 
folder and its subfolders and directories; e.g., in a terminal, change to
the Sites folder and 'chmod -RE 0777 sch'. (you may need to use sudo,
e.g., 'sudo chmod -RE 0777 sch').
