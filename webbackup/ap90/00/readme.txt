readme.txt for ap90web download.  
May 24, 2014
Jim Funderburk
...
a user to install on a local computer a web-browser display of ap90 data.

unzip the ap90web.zip archive. This creates a folder called 'web'.
Rename 'web' to something such as 'ap90'.
...
Move the 'ap90' folder to the 'htdocs' subfolder of your Apache installation.
...
Then, use the url <home>/ap90/ to get the home page of the ap90 displays;
for instance, http://127.0.0.1:4001/ap90/.

For server2go, you may need to slightly adjust the pms_config.ini file to
the one contained in the server2go subdirectory of this ap90web download.


For Mac installation, move the ap90 folder to the Sites folder, and make 
sure web sharing is turned on.
You may need to changed the permissions on the ap90 
folder and its subfolders and directories; e.g., in a terminal, change to
the Sites folder and 'chmod -RE 0777 ap90'. (you may need to use sudo,
e.g., 'sudo chmod -RE 0777 ap90').
