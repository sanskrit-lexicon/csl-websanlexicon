...
//echo "0#<p>dbg: $opt_stransLit,$opt_sword</p>";
...
$wordin="";
if ($word !="") {
 // for skd, the text words are in devanagari also!
 //$wordchrs = preg_split ('/[^a-zA-Z.*?+|]/',$word);
 //$word = join('',$wordchrs);
 //$wordin = $word;
 $slpword = translate_string2SLP($opt_stransLit,$word);
 ...
 $tempar = matchkey($fp,$lastLnum,$search_regexp,$max,$search_opt,$slpword);
 $matches = $tempar['ans'];
