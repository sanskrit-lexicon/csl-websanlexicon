...
// May 31, 2017 copied from acc/web/webtc/disp.php
*/
...
function line_adjust($line) {
 ...
 // It might be better to do this transformation either in xxx.txt or xxx.xml
 $line = preg_replace('/--/','&#8212;',$line);
...
function sthndl($xp,$el,$attribs) {
 ...
 global $parentEl;
  ...
  if (preg_match('/^H.+$/',$el)) {
   ...
   $row .= "<sup>";
  } else if ($el == "g") { // Greek text. 1 instance in yat.
  } else {
    $row .= "<br/>&lt;$el&gt;";
  }
