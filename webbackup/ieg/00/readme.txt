readme.txt for iegweb download.  
May 1, 2014
Jim Funderburk
...
a user to install on a local computer a web-browser display of ieg data.

unzip the iegweb.zip archive. This creates a folder called 'web'.
Rename 'web' to something such as 'ieg'.
...
Move the 'ieg' folder to the 'htdocs' subfolder of your Apache installation.
...
Then, use the url <home>/ieg/ to get the home page of the ieg displays;
for instance, http://127.0.0.1:4001/ieg/.

For server2go, you may need to slightly adjust the pms_config.ini file to
the one contained in the server2go subdirectory of this iegweb download.


For Mac installation, move the ieg folder to the Sites folder, and make 
sure web sharing is turned on.
You may need to changed the permissions on the ieg 
folder and its subfolders and directories; e.g., in a terminal, change to
the Sites folder and 'chmod -RE 0777 ieg'. (you may need to use sudo,
e.g., 'sudo chmod -RE 0777 ieg').
