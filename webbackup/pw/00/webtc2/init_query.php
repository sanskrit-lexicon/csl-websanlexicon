...
function query_line($x) {
 ...
 //$x = mb_ereg_replace("ä","a",$x);
 $umlauts = array("Ä","Ö","Ü","ß","ä","é","ê","ë","ö","ü");
 $nonumlauts = array("A","U","U","s","a","e","e","e","o","u");
 for($i=0;$i<count($umlauts);$i++) {
  $x = mb_ereg_replace($umlauts[$i],$nonumlauts[$i],$x);
 }
 $x = preg_replace('|ƒPage.*?ƒ|','',$x);
