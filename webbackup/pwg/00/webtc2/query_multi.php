...
function print_table($filter,$key,$ntab,$nmatchesin,$matchesin,$accent) {
 ...
 dbgprintqm($dbg,"print_table: calling basicDisplaySetAccent\n");
 basicDisplaySetAccent($accent);
 dbgprintqm($dbg,"print_table: calling basicDisplay\n");
 $table = basicDisplay($key,$matches,$filter);
 dbgprintqm($dbg,"print_table: Transcoding\n");
 $table1 = transcoder_processElements($table,"slp1",$filter,"SA");
 echo $table0;
 echo $table1;
}
function dbgprintqm($dbg,$text) {
 if (!$dbg) {return;}
