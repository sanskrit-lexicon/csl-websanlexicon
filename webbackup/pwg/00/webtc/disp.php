...
function basicDisplay($key,$matches,$filterin) {
 ...
 while($i<$ntot) {
  ...
  $line=trim($line);
  $l0=strlen($line);
  dbgprintdisp($dbg,"BasicDisplay: i=$i, calling line_adjust with line=\n$line\n");
  $line=line_adjust($line);
  dbgprintdisp($dbg,"BasicDisplay: back from line_adjust\n");
  $row = "";
  $row1 = "";
...
//   echo "<p>The underlying data for this line:<br/>$line</p>\n";
  ...
  dbgprintdisp($dbg,"BasicDisplay: calling xml_parse\n");
...
//                    xml_error_string(xml_get_error_code($p)));
  ...
  xml_parser_free($p);
  dbgprintdisp($dbg,"BasicDisplay: back from xml_parse\n");
...
function line_adjust($line) {
 ...
 $dbg=false;
 dbgprintdisp($dbg,"Begin line_adjust\n");
 $line = preg_replace('/¦/',' ',$line);
 ...
 */
 dbgprintdisp($dbg,"Begin line_adjust: ls_callback\n");
...
function ls_callback($matches) {
 ...
 # 12-26-2017. Add lshead, so as to be able to style
 ...
 global $parentEl;
  ...
  if (preg_match('/^H.+$/',$el)) {
   ...
   $row .= "<sup>";
    ...
    $n=$attribs['n'];
    $style="";
    if (($n == "Russian")||($n == "Old-Church-Slavonic")) {
     $row .= " ($n) ";   // temporary 01-03-2018
     //$row .= "<span>";
     $style=" style='font-family: oldstandard;'";
    }
    $row .= "<span$style>";
  } else if ($el == "lb") {
    $row .= "<br/>";
   ...
   // no rendering
    ...
    if (isset($attribs['n'])) {
     $tran = $attribs['n'];
     dbgprintdisp(false," sthndl. ab. tran=$tran\n");
