...
/* webtc/dal_sqlite.php  
  ...
  dal_ae1(key) from 'ae' return array of records (key1,lnum1,data1)
            where key1 = key
  dal_ae2(L1,L2) from 'ae' return array of records (key1,lnum1,data1)
            where L1 <= lnum <= L2
*/
...
function dal_ae_sql($sql) {
// General query on 'ae' database. Table assumed in $sql
$db = "../sqlite/ae.sqlite";
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_ae1($key) {
...
$db = "sqlite:../sqlite/ae.sqlite";
// Create (connect to) SQLite database in file
$file_db = new PDO($db);
// Set errormode to exceptions
$file_db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
$sql = "select * from ae where key='$key' order by lnum";
$result = $file_db->query($sql);
//echo "<p>Using PDO...</p>";
$ansarr=array();
foreach($result as $m) {
...
function dal_ae2($L1,$L2) {
// returns an array of records, one for each L-value in the range
// $L1 <= $L <= $L2
// each record is an array with three elements: key,lnum,data
$db = "../sqlite/ae.sqlite";
$sql="select * from ae where  $L1 <= lnum and lnum <= $L2  order by lnum"; 
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_ae3($key) {
// returns an array of records, which start like $key
// each record is an array with three elements: key,lnum,data
$db = "../sqlite/ae.sqlite";
$sql = "select * from ae where key LIKE '$key%' order by lnum";
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_ae4a($lnum0,$max) {
$sql = "select * from ae where (lnum < '$lnum0') order by lnum DESC LIMIT $max";
$recarr = dal_ae_sql($sql);
return $recarr;
}
function dal_ae4b($lnum0,$max) {
$sql = "select * from ae where ('$lnum0' < lnum) order by lnum LIMIT $max";
//$sql = "select * from `ae` where \"$lnum0\" < `lnum`  order by `lnum`  LIMIT $max";
$recarr = dal_ae_sql($sql);
return $recarr;
}
