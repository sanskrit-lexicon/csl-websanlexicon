readme.txt for shsweb download.  
May 10, 2014
Jim Funderburk
...
a user to install on a local computer a web-browser display of shs data.

unzip the shsweb.zip archive. This creates a folder called 'web'.
Rename 'web' to something such as 'shs'.
...
Move the 'shs' folder to the 'htdocs' subfolder of your Apache installation.
...
Then, use the url <home>/shs/ to get the home page of the shs displays;
for instance, http://127.0.0.1:4001/shs/.

For server2go, you may need to slightly adjust the pms_config.ini file to
the one contained in the server2go subdirectory of this shsweb download.


For Mac installation, move the shs folder to the Sites folder, and make 
sure web sharing is turned on.
You may need to changed the permissions on the shs 
folder and its subfolders and directories; e.g., in a terminal, change to
the Sites folder and 'chmod -RE 0777 shs'. (you may need to use sudo,
e.g., 'sudo chmod -RE 0777 shs').
