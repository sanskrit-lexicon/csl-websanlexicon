...
/* webtc/dal_sqlite.php  
  ...
  dal_shs1(key) from 'shs' return array of records (key1,lnum1,data1)
            where key1 = key
  dal_shs2(L1,L2) from 'shs' return array of records (key1,lnum1,data1)
            where L1 <= lnum <= L2
*/
...
function dal_shs_sql($sql) {
// General query on 'shs' database. Table assumed in $sql
$db = "../sqlite/shs.sqlite";
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_shs1($key) {
...
$db = "sqlite:../sqlite/shs.sqlite";
// Create (connect to) SQLite database in file
$file_db = new PDO($db);
// Set errormode to exceptions
$file_db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
$sql = "select * from shs where key='$key' order by lnum";
$result = $file_db->query($sql);
//echo "<p>Using PDO...</p>";
$ansarr=array();
foreach($result as $m) {
...
function dal_shs2($L1,$L2) {
// returns an array of records, one for each L-value in the range
// $L1 <= $L <= $L2
// each record is an array with three elements: key,lnum,data
$db = "../sqlite/shs.sqlite";
$sql="select * from shs where  $L1 <= lnum and lnum <= $L2  order by lnum"; 
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_shs3($key) {
// returns an array of records, which start like $key
// each record is an array with three elements: key,lnum,data
$db = "../sqlite/shs.sqlite";
$sql = "select * from shs where key LIKE '$key%' order by lnum";
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_shs4a($lnum0,$max) {
$sql = "select * from shs where (lnum < '$lnum0') order by lnum DESC LIMIT $max";
$recarr = dal_shs_sql($sql);
return $recarr;
}
function dal_shs4b($lnum0,$max) {
$sql = "select * from shs where ('$lnum0' < lnum) order by lnum LIMIT $max";
//$sql = "select * from `shs` where \"$lnum0\" < `lnum`  order by `lnum`  LIMIT $max";
$recarr = dal_shs_sql($sql);
return $recarr;
}
