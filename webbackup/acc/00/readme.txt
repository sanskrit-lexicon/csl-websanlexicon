readme.txt for accweb download.  
June 10, 2014
Jim Funderburk
...
a user to install on a local computer a web-browser display of acc data.

unzip the accweb.zip archive. This creates a folder called 'web'.
Rename 'web' to something such as 'acc'.
...
Move the 'acc' folder to the 'htdocs' subfolder of your Apache installation.
...
Then, use the url <home>/acc/ to get the home page of the acc displays;
for instance, http://127.0.0.1:4001/acc/.

For server2go, you may need to slightly adjust the pms_config.ini file to
the one contained in the server2go subdirectory of this accweb download.


For Mac installation, move the acc folder to the Sites folder, and make 
sure web sharing is turned on.
You may need to changed the permissions on the acc 
folder and its subfolders and directories; e.g., in a terminal, change to
the Sites folder and 'chmod -RE 0777 acc'. (you may need to use sudo,
e.g., 'sudo chmod -RE 0777 acc').
