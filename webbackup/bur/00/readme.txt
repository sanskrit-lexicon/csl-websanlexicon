readme.txt for burweb download.  
June 21, 2013. Jim Funderburk
...
a user to install on a local computer a display of bur data.

unzip the burweb.zip archive. This creates a folder called 'web'.
Rename 'web' to something such as 'bur'.
...
Move the 'bur' folder to the 'htdocs' subfolder of your Apache installation.
...
Then, use the url <home>/bur/ to get the home page of the bur displays;
for instance, http://127.0.0.1:4001/bur/.

For server2go, you may need to slightly adjust the pms_config.ini file to
the one contained in the server2go subdirectory of this burweb download.


For Mac installation, move the bur server to the Sites folder, and make 
sure web sharing is turned on.
You may need to changed the permissions on the bur 
folder and its subfolders and directories; e.g., in a terminal, change to
the Sites folder and 'chmod -RE 0777 bur'. (you may need to use sudo,
e.g., 'sudo chmod -RE 0777 bur').
