...
//ejf July 25, 2017. Problem with list display for 'DarmeRa' (L=99930.1):
//    Only one record occurs in the list.
...
$recarr = dal_mw3($key); // records LIKE key%
$matches=array();
$nmatches=0;
$keylen = strlen($key);
$more=true;
...
$dbg = false;
dbgprint($dbg,"listhier/list_prev 1: $key0,$lnum0,$nprev\n");
$ans = array();
if ($nprev <= 0) {return $ans;}
$max = 5 * $nprev;  // 5 is somewhat arbitrary.
$recarr = dal_mw4a($lnum0,$max);
dbgprint($dbg,"listhier/list_prev 2: " . count($recarr) . " records\n");
// 1. get records to be displayed
$matches = list_filter($lnum0,$recarr);
// 2. get the last $nprev records for return
$nmatches = count($matches);
dbgprint($dbg,"listhier/list_prev 3: " . $nmatches . " records after list_filter\n");
...
$dbg=false;
$matches=array();
$recarr0 = dal_mw2($lnum0,$lnum0); 
dbgprint($dbg,"list_filter: lnum0=$lnum0; recarr0 has " . count($recarr0) . " records\n");
