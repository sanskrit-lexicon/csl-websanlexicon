...
a user to install on a local computer a web-browser display of md data.

unzip the mdweb.zip archive. This creates a folder called 'web'.
Rename 'web' to something such as 'md'.
...
Move the 'md' folder to the 'htdocs' subfolder of your Apache installation.
...
Then, use the url <home>/md/ to get the home page of the md displays;
for instance, http://127.0.0.1:4001/md/.

For server2go, you may need to slightly adjust the pms_config.ini file to
the one contained in the server2go subdirectory of this mdweb download.


For Mac installation, move the md folder to the Sites folder, and make 
sure web sharing is turned on.
You may need to changed the permissions on the md 
folder and its subfolders and directories; e.g., in a terminal, change to
the Sites folder and 'chmod -RE 0777 md'. (you may need to use sudo,
e.g., 'sudo chmod -RE 0777 md').
