...
/* webtc/dal_sqlite.php  ejf Oct 31, 2012
  Modified May 14, 2013.  dal_stc1 routine uses php PDO.
  This is the only function used in the stc display.

  This collects all the database access code used by
  webtc and webtc5wa.
  It assumes the data access is via sqlite.
  There are several routines:
  dal_stc1(key) from 'stc' return array of records (key1,lnum1,data1)
            where key1 = key
  dal_stc2(L1,L2) from 'stc' return array of records (key1,lnum1,data1)
            where L1 <= lnum <= L2
...
function dal_stc_sql($sql) {
// General query on 'stc' database. Table assumed in $sql
$db = "../sqlite/stc.sqlite";
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_stc1($key) {
...
$db = "sqlite:../sqlite/stc.sqlite";
// Create (connect to) SQLite database in file
$file_db = new PDO($db);
// Set errormode to exceptions
$file_db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
$sql = "select * from stc where key='$key' order by lnum";
$result = $file_db->query($sql);
//echo "<p>Using PDO...</p>";
$ansarr=array();
foreach($result as $m) {
...
function dal_stc2($L1,$L2) {
// returns an array of records, one for each L-value in the range
// $L1 <= $L <= $L2
// each record is an array with three elements: key,lnum,data
$db = "../sqlite/stc.sqlite";
$sql="select * from stc where  $L1 <= lnum and lnum <= $L2  order by lnum"; 
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_stc3($key) {
// returns an array of records, which start like $key
// each record is an array with three elements: key,lnum,data
$db = "../sqlite/stc.sqlite";
$sql = "select * from stc where key LIKE '$key%' order by lnum";
$recarr = dal_sqlite($db,$sql);
return $recarr;
}
function dal_stc4a($lnum0,$max) {
$sql = "select * from stc where (lnum < '$lnum0') order by lnum DESC LIMIT $max";
$recarr = dal_stc_sql($sql);
return $recarr;
}
function dal_stc4b($lnum0,$max) {
$sql = "select * from stc where ('$lnum0' < lnum) order by lnum LIMIT $max";
//$sql = "select * from `stc` where \"$lnum0\" < `lnum`  order by `lnum`  LIMIT $max";
$recarr = dal_stc_sql($sql);
return $recarr;
}
