...
// May 22, 2017. display_outopt4 modified to remove duplicate keys
$dbg = FALSE;
...
function dbgprint($dbg,$string) {
 if (! $dbg) {return;}
 $filename="temp_debug.txt";
 $fp = fopen($filename,"a");
 fwrite($fp,$string);
 fclose($fp);
}
function remove_duplicate_keys($lines) {
 $dbg=false;
 $nlines = count($lines);
 dbgprint($dbg,"remove_duplicate_keys: $nlines lines\n");
 // remove lines with a duplicate key, Result is $lines1
 $lines1= array();
 $keys = array();
 foreach($lines as $x) {
  if (preg_match('/^(.*?)\t(.*?)$/',$x,$matches)) {
   ...
   $key = trim($key);
   dbgprint($dbg,"key=$key, x=$x\n");
   $asval = array_search($key,$keys);
   dbgprint($dbg,"key=$key,  asval=$asval\n");
   /* this test condition for array_search is very delicate.
      When $asval === false, then $key is not in $keys
   */
   if ($asval !== false) { 
    continue;
   }
   dbgprint($dbg,"asval=$asval\n");
   $lines1[] = $x;
   $keys[] = $key;
  }
 }
...
function display_outopt4 ($lines,$word,$search_regexp) {
 $lines1 = remove_duplicate_keys($lines);
 $nx=0;
