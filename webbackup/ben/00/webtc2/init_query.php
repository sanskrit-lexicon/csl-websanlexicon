...
function query_line($x) {
 ...
 // (a1) remove remaining extended ascii coded as entities
  $x = preg_replace("|&#x....;|","",$x); // 
 // (a2) change unicode upper/lower cedilla to c

 $unicodeChars = array(
  array('\u00c7','C'), // LATIN CAPITAL LETTER C WITH CEDILLA
  array('\u00e7','c'), // LATIN SMALL LETTER C WITH CEDILLA
  array('\u00a4',''),  // CURRENCY SIGN
  array('\u00a6',''),  // BROKEN BAR
  array('\u00ba',''),  // MASCULINE ORDINAL INDICATOR
  array('\u00c6','AE'),  // LATIN CAPITAL LETTER AE
  array('\u00e6','ae'),  // LATIN SMALL LETTER AE
  array('\u0153','oe'),  // LATIN SMALL LIGATURE OE
  array('\u2018',''),  // LEFT SINGLE QUOTATION MARK
  array('\u2019',''),  // RIGHT SINGLE QUOTATION MARK
  array('\u201e',''),  // DOUBLE LOW-9 QUOTATION MARK
  array('\u2020','')  // DAGGER
 );
 foreach($unicodeChars as $a) {
  list($unicodeChar,$c) = $a;
  $temp = json_decode('"'.$unicodeChar.'"');
  $x = preg_replace("|$temp|",$c,$x);
 }
