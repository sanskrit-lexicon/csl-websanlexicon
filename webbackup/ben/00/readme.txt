readme.txt for benweb download.  
March 27, 2014
Jim Funderburk
...
a user to install on a local computer a web-browser display of ben data.

unzip the benweb.zip archive. This creates a folder called 'web'.
Rename 'web' to something such as 'ben'.
...
Move the 'ben' folder to the 'htdocs' subfolder of your Apache installation.
...
Then, use the url <home>/ben/ to get the home page of the ben displays;
for instance, http://127.0.0.1:4001/ben/.

For server2go, you may need to slightly adjust the pms_config.ini file to
the one contained in the server2go subdirectory of this benweb download.


For Mac installation, move the ben folder to the Sites folder, and make 
sure web sharing is turned on.
You may need to changed the permissions on the ben 
folder and its subfolders and directories; e.g., in a terminal, change to
the Sites folder and 'chmod -RE 0777 ben'. (you may need to use sudo,
e.g., 'sudo chmod -RE 0777 ben').
