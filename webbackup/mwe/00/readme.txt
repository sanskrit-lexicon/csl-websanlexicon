readme.txt for mweweb download.  
October 25, 2013. Jim Funderburk
...
a user to install on a local computer a web-browser display of mwe data.

unzip the mweweb.zip archive. This creates a folder called 'web'.
Rename 'web' to something such as 'mwe'.
...
Move the 'mwe' folder to the 'htdocs' subfolder of your Apache installation.
...
Then, use the url <home>/mwe/ to get the home page of the mwe displays;
for instance, http://127.0.0.1:4001/mwe/.

For server2go, you may need to slightly adjust the pms_config.ini file to
the one contained in the server2go subdirectory of this mweweb download.


For Mac installation, move the mwe folder to the Sites folder, and make 
sure web sharing is turned on.
You may need to changed the permissions on the mwe 
folder and its subfolders and directories; e.g., in a terminal, change to
the Sites folder and 'chmod -RE 0777 mwe'. (you may need to use sudo,
e.g., 'sudo chmod -RE 0777 mwe').
