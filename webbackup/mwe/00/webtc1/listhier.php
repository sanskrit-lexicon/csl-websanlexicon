<?php
////scans/MWEScan/2013/web/webtc1/listhier.php
//ejf May 28, 2013
...
while($i < count($listmatches)) {
 ...
 if ($i == 0) {
  ...
  $out1 = "$spc<a  onclick='getWordlistUp_keyboard(\"$key2\");'><span style='$c'>&#x25B2;</span></a><br/>\n";  
  $table .= $out1;
 }
 $i++;
 ...
 $out1 = "$spc<a  onclick='getWordAlt_keyboard(\"$key2\");'><span style='$c'>$key2show</span>$hom2</a>$xtraskip<br/>\n";
 ...
 if ($i == count($listmatches)) {
  ...
  $out1 = "$spc<a  onclick='getWordlistDown_keyboard(\"$key2\");'><span style='$c'>&#x25BC;</span></a><br/>\n";  
  $table .= $out1;
 }
...
$recarr = dal_mwe3($key); // records LIKE key%
$matches=array();
$nmatches=0;
$keylen = strlen($key);
$more=true;
