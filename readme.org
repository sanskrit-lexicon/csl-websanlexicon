...
# See https://stackoverflow.com/questions/23698183/how-to-force-cp-to-overwrite-directory-instead-of-creating-another-one-inside
# for explanation of T option in the copy.
# a) If the target directory does not exist, then it is created
# b) If the target directory DOES exist, then all source files are
#    copied into it.
#    In this case, cp -r (without the T) creates a copy of the source
#    within the target, which is not desired
